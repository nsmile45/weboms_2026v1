...
function chkColor(chk: string) {
  if (chk === '')  return 'bg-[#e8f0fe] text-[#2b579a] border border-[#b8cde8]'
  if (chk === '0') return 'bg-[#e6f4ea] text-[#1e7e34] border border-[#b7dfc4]'
  if (chk === '9') return 'bg-[#fce8e8] text-[#c62828] border border-[#f5b7b7]'
  return 'bg-[#fff3e0] text-[#e65100] border border-[#ffd49a]'
}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
      <div className="bg-white rounded-lg shadow-2xl w-[620px] max-h-[80vh] flex flex-col" onClick={e=>e.stopPropagation()}>
        {/* 헤더 */}
        <div className="flex items-center justify-between px-3 py-1.5 bg-[#2b579a] text-white rounded-t-sm shrink-0">
          <span className="text-[13px] font-medium">서점 검색</span>
          <button onClick={onClose} className="text-blue-200/70 hover:text-white text-sm leading-none">✕</button>
        </div>
        {/* 검색 조건 */}
        <div className="flex items-center gap-1.5 px-3 py-2 border-b border-[#dde4ed] shrink-0">
          <span className="text-[13px] text-[#5a6a7e] shrink-0">서점명</span>
          <input
            ref={inputRef}
            value={custNm}
            ...
            placeholder="서점명"
            className="flex-1 h-6 px-2 text-[13px] border border-[#b8c4d4] rounded-sm focus:outline-none focus:ring-1 focus:ring-[#2b579a]/30 focus:border-[#2b579a]"
          />
          <span className="text-[13px] text-[#5a6a7e] shrink-0">서점코드</span>
          <input
            value={metaxNo}
            onChange={e => setMetaxNo(e.target.value)}
            onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); search(custNm, metaxNo) } }}
            placeholder="서점코드"
            className="w-28 h-6 px-2 text-[13px] border border-[#b8c4d4] rounded-sm focus:outline-none focus:ring-1 focus:ring-[#2b579a]/30 focus:border-[#2b579a]"
          />
          <button onClick={() => search(custNm, metaxNo)}
            className="h-8 px-3 text-[13px] font-medium bg-[#2b579a] text-white rounded-sm hover:bg-[#1e3a5f] transition-colors shrink-0">
            조회
          </button>
        </div>
        {/* 컬럼 헤더 */}
        <div className="grid grid-cols-[2fr_1fr_1fr_1fr] px-2 py-1 bg-[#34568B] border-b border-[#c8d0e0] text-[13px] font-medium text-white shrink-0">
          <span>서점명</span>
          <span>서점코드</span>
          <span>배송구분</span>
          <span>전화번호</span>
        ...
        <div className="flex-1 overflow-y-auto">
          {loading ? (
            ...
            <div
              ...
              className={`grid grid-cols-[2fr_1fr_1fr_1fr] px-2 py-1 text-[13px] border-b border-[#dde4ed] cursor-pointer transition-colors ${
                selectedIdx === i ? 'bg-[#FDF5E6] text-[#1e3a5f] font-medium' : i%2===0 ? 'bg-white hover:bg-[#e8f0fe]' : 'bg-[#f6f8fb] hover:bg-[#e8f0fe]'
              } ${c.CHUL_BLOCK_YN === 'Y' || c.USE_YN !== 'Y' ? 'opacity-50' : ''}`}
            >
              <span className="font-medium truncate">{c.MECUST_NM}</span>
              <span className={selectedIdx === i ? 'text-[#5a6a7e]' : 'text-slate-800'}>{c.METAX_NO}</span>
              <span className={selectedIdx === i ? 'text-[#5a6a7e]' : 'text-slate-800'}>{c.BESONG_GB}</span>
              <span className={selectedIdx === i ? 'text-[#5a6a7e]' : 'text-slate-800'}>{c.TEL_NO}</span>
            </div>
          ))}
        </div>
        {/* 푸터 */}
        <div className="flex items-center justify-between px-3 py-1.5 bg-[#f4f6f8] border-t border-[#dde4ed] rounded-b-sm shrink-0">
          <span className="text-[13px] text-[#8a9ab0]">{results.length > 0 ? `총 ${results.length}건 · Enter로 선택` : ''}</span>
          <button onClick={onClose} className="h-8 px-3 text-[13px] border border-[#b8c4d4] rounded-sm bg-white hover:bg-[#eef2f7] transition-colors">닫기</button>
        </div>
      </div>
    </div>
  )
...
export default function P121Page() {
  ...
  const meGbRef = useRef<HTMLSelectElement>(null)
  const pmRemkRef = useRef<HTMLInputElement>(null)
  const bookNameRef = useRef<HTMLInputElement>(null)
  const outDangaRef = useRef<HTMLInputElement>(null)
  const odQtyRef = useRef<HTMLInputElement>(null)
  const outRateRef = useRef<HTMLInputElement>(null)
  const pubRemkRef = useRef<HTMLInputElement>(null)
  ...
  function applyCust(c: CustResult) {
    ...
    setTimeout(()=>meGbRef.current?.focus(), 0)
  }
  ...
  function applyBook(b: BookResult) {
    ...
    setTimeout(()=>odQtyRef.current?.focus(), 0)
  }
  ...
  // ─── 공통 인풋 스타일 ─────────────────────────────────────────────
  const inp  = 'h-8 px-2 text-[13px] border border-[#b8c4d4] rounded-sm bg-white w-full focus:outline-none focus:ring-1 focus:ring-[#2b579a]/30 focus:border-[#2b579a] transition-colors'
  const inpD = 'h-8 px-2 text-[13px] border border-[#d8dfe8] rounded-sm bg-[#f4f6f8] w-full text-[#8a9ab0] cursor-default'
  const sel  = 'h-8 px-2 text-[13px] border border-[#b8c4d4] rounded-sm bg-white w-full focus:outline-none focus:ring-1 focus:ring-[#2b579a]/30 focus:border-[#2b579a] transition-colors'
  const selD = 'h-8 px-2 text-[13px] border border-[#d8dfe8] rounded-sm bg-[#f4f6f8] w-full text-[#8a9ab0] cursor-default'

  return (
    <div className="flex flex-col h-full bg-[#eef2f7] text-[13px] select-none">

      {/* ── 상단 툴바 ───────────────────────────────────────────────── */}
      <div className="flex items-center gap-1 px-2 py-1 bg-[#f0f4f8] border-b border-[#c8d1dc]">
        ...
        <span className={`text-[13px] font-medium px-2 py-0.5 rounded-sm ${master.SUBL_NO ? chkColor(master.CHK_GB) : 'text-[#8a9ab0]'}`}>
          {statusMsg}
        </span>
      </div>
      ...
      <div className="flex flex-col flex-1 min-h-0 p-1.5 gap-1.5">
        ...
        <div className="flex gap-2">
          ...
          {/* 마스터정보 */}
          <fieldset className="w-[470px] shrink-0 border border-[#b8c4d4] bg-white rounded-sm px-2 pt-0 pb-1.5">
            <legend className="text-[12px] font-semibold text-[#2b579a] px-1 uppercase tracking-wide">마스터정보</legend>
            <div className="grid grid-cols-[auto_1fr_auto_1fr] gap-x-1.5 gap-y-1 items-center">
              ...
              <div className="relative flex items-center gap-1 col-span-3">
                    ...
                    <div className="absolute top-full left-0 z-50 bg-white border border-gray-200 rounded-lg shadow-xl w-80 max-h-56 overflow-y-auto mt-1">
                      {custResults.map(c=>(
                        <div key={c.METAX_NO} onMouseDown={()=>applyCust(c)}
                          ...
                          <span className="text-gray-400 text-[13px]">{c.METAX_NO}</span>
                        </div>
                      ))}
                    </div>
                  )}
                ...
                <input value={master.METAX_NO} readOnly placeholder="코드"
                  className="h-8 px-2 text-xs border border-slate-100 rounded-md bg-slate-50 text-slate-400 cursor-default flex-shrink-0 w-[4.5rem]" />
                {/* 검색 버튼 */}
                <button onClick={()=>setShowCustModal(true)}
                  className="h-8 px-2.5 text-xs font-semibold border border-blue-300 rounded-md bg-blue-50 text-blue-600 hover:bg-blue-100 transition-colors whitespace-nowrap flex-shrink-0">검색</button>
                <div className="w-px h-4 bg-gray-200 mx-0.5 flex-shrink-0" />
                {/* 전표 */}
                <L>전표</L>
                <input value={master.SUBL_NO || ''} readOnly
                  className="h-8 px-2 text-xs border border-slate-100 rounded-md bg-slate-50 text-slate-400 cursor-default flex-shrink-0 w-[4.5rem]" />
                <span className={`text-[13px] px-2 py-0.5 rounded-full font-semibold whitespace-nowrap ${chkColor(master.CHK_GB)}`}>
                  {chkLabel(master.CHK_GB, codes)}
                </span>
              </div>

              <L>매출구분</L>
              <select ref={meGbRef} value={master.ME_GB} disabled={!canEdit}
                onChange={e=>{setMaster(p=>({...p,ME_GB:e.target.value})); setTimeout(()=>pmRemkRef.current?.focus(), 0)}}
                onKeyDown={e=>{ if(e.key==='Enter'){ e.preventDefault(); pmRemkRef.current?.focus() }}}
                className={canEdit ? sel : selD}>
          ...
          {/* 택배정보 */}
          <fieldset className="w-[340px] border border-[#b8c4d4] bg-white rounded-sm px-2 pt-0 pb-1.5">
            <legend className="text-[12px] font-semibold text-[#2b579a] px-1 uppercase tracking-wide">택배정보</legend>
            <div className="grid grid-cols-[auto_1fr] gap-x-1.5 gap-y-1 items-center">
              ...
              <div className="flex gap-0.5">
                ...
                <button className="h-8 px-1.5 text-xs border border-slate-300 rounded bg-slate-50 hover:bg-slate-200">O</button>
              </div>
              <L>상세주소</L>
          ...
          {/* 부가기능 */}
          <div className="w-[130px] border border-[#b8c4d4] bg-white rounded-sm px-1.5 py-1.5 flex flex-col gap-1">
            <div className="text-[12px] font-semibold text-[#2b579a] uppercase tracking-wide mb-0.5">부가기능</div>
            <button onClick={()=>setShowSearchModal(true)}
              className="h-8 text-[13px] bg-[#2b579a] text-white rounded-sm hover:bg-[#1e3a5f] font-medium transition-colors">전표조회 및 전송</button>
            <button onClick={saveMaster} disabled={!master.SUBL_NO || !canEdit}
              className="h-8 text-[13px] border border-[#b8c4d4] bg-[#f4f6f8] text-[#5a6a7e] rounded-sm hover:bg-[#e8ecf0] disabled:opacity-40 transition-colors">마스터 수정</button>
            <button disabled className="h-8 text-[13px] border border-[#d8dfe8] bg-[#f4f6f8] text-[#b8c4d4] rounded-sm cursor-not-allowed">매출을 재계산</button>
            <button disabled className="h-8 text-[13px] border border-[#d8dfe8] bg-[#f4f6f8] text-[#b8c4d4] rounded-sm cursor-not-allowed">세트도서 등록</button>
          </div>

          {/* 도서검색 안내 */}
          <div className="flex-1 min-w-[160px] border border-[#b8c4d4] bg-white rounded-sm px-2 py-1.5">
            <div className="text-[12px] font-semibold text-[#2b579a] mb-1">도서 검색 방법</div>
            <div className="space-y-0 text-[12px] text-[#8a9ab0] leading-4">
              <div>① 도서코드</div>
              <div>② 바코드 (ISBN)</div>
              <div>③ 도서명 중간자</div>
              <div>④ 도서명 + 저자</div>
        ...
        {/* ── 명세서비고 / 삭제사유 ──────────────────────────────── */}
        <div className="flex items-center gap-2 bg-white border border-[#b8c4d4] rounded-sm px-2 py-1">
          <span className="text-[13px] text-[#5a6a7e] whitespace-nowrap w-14">명세서비고</span>
          <input ref={pmRemkRef} value={master.PM_REMK} readOnly={!canEdit}
            onChange={e=>setMaster(p=>({...p,PM_REMK:e.target.value}))}
            onKeyDown={e=>{ if(e.key==='Enter'){ e.preventDefault(); bookNameRef.current?.focus() }}}
            className={`flex-1 ${canEdit ? inp : inpD}`} />
          <span className="text-[13px] text-[#5a6a7e] whitespace-nowrap w-12">삭제사유</span>
          <input value={master.MIS_DESC} readOnly className={`flex-1 ${inpD}`} />
        </div>

        {/* ── 도서 그리드 ─────────────────────────────────────────── */}
        <div className="flex-1 min-h-0 bg-white border border-[#b8c4d4] rounded-sm overflow-hidden flex flex-col">
          {/* 합계 표시 */}
          <div className="flex items-center gap-3 px-2 py-1 bg-white border-b border-[#b8c4d4] text-[13px]">
            <span className="font-medium text-[#5a6a7e] uppercase tracking-wide text-[12px]">주문도서</span>
            <span className="font-bold text-[#1e3a5f]">{lines.length}<span className="font-normal text-[#8a9ab0] ml-0.5">종</span></span>
            <span className="font-bold text-[#1e3a5f]">{totalQty.toLocaleString()}<span className="font-normal text-[#8a9ab0] ml-0.5">부</span></span>
            <span className="font-bold text-[#2b579a]">{totalAmt.toLocaleString()}<span className="font-normal text-[#8a9ab0] ml-0.5">원</span></span>
          </div>
          <div className="flex-1 overflow-auto">
            <table className="w-full text-[13px] border-collapse [&_td]:border-r [&_td]:border-[#d0d8e4] [&_td:last-child]:border-r-0" style={{minWidth:900}}>
              <thead className="sticky top-0 z-10">
                <tr className="bg-[#E7EBF5] text-black">
                  {['번호','도서코드','도서명','바코드','저자','정가','물류재고','반품재고','출고대기','출고가능','부수','%','주문금액','도서비고(출판사도서관...)','삭제'].map(h=>(
                    <th key={h} className="px-1 py-1.5 text-center font-medium whitespace-nowrap border-r border-[#c8d0e0] last:border-r-0">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {lines.map((ln,i)=>{
                  ...
                  return (
                    <tr key={ln.SUBL_SEQ}
                      className={`border-b border-[#dde4ed] ${isEdit ? 'bg-[#e8f0fe]' : i%2===0 ? 'bg-white' : 'bg-[#f6f8fb]'} hover:bg-[#e8f0fe]/60 transition-colors`}>
                      <td className="px-1.5 py-1.5 text-center text-[#8a9ab0] border-r border-[#dde4ed]">{i+1}</td>
                      {isEdit ? (
                        <>
                          <td className="px-1 py-0.5 border-r border-[#dde4ed]">{editLine.BK_CD}</td>
                          <td className="px-1 py-0.5 border-r border-[#dde4ed] relative min-w-[140px]">
                          ...
                          <td className="px-1 py-0.5 border-r border-[#dde4ed]">{editLine.BAR_CD}</td>
                          <td className="px-1 py-0.5 border-r border-[#dde4ed]">{editLine.WRITER}</td>
                          <td className="px-1 py-0.5 border-r border-[#dde4ed]">
                          ...
                          <td className="px-1 py-0.5 border-r border-[#dde4ed]">{editLine.BK_QTY10}</td>
                          <td className="px-1 py-0.5 border-r border-[#dde4ed]">-</td>
                          <td className="px-1 py-0.5 border-r border-[#dde4ed]">-</td>
                          <td className="px-1 py-0.5 border-r border-[#dde4ed]">{editLine.JG_QTY}</td>
                          <td className="px-1 py-0.5 border-r border-[#dde4ed]">
                          ...
                          <td className="px-1 py-0.5 border-r border-[#dde4ed] text-right">{editLine.OD_AMT.toLocaleString()}</td>
                          <td className="px-1 py-0.5 border-r border-[#dde4ed]">
                            <input value={editLine.PUB_REMK} onChange={e=>setEditLine(p=>({...p,PUB_REMK:e.target.value}))}
                              className="h-5 px-1 text-xs border border-blue-400 rounded w-full" />
                          </td>
                          <td className="px-1 py-0.5 text-center">
                            <button onClick={saveEditLine} className="text-[12px] px-1.5 py-0.5 bg-blue-600 text-white rounded mr-0.5">저장</button>
                            <button onClick={()=>setEditIdx(null)} className="text-[12px] px-1 py-0.5 bg-slate-300 rounded">취소</button>
                          </td>
                        </>
                      ) : (
                        <>
                          <td className="px-1.5 py-0.5 text-center border-r border-[#dde4ed]">{ln.BK_CD}</td>
                          <td className="px-1.5 py-0.5 border-r border-[#dde4ed] max-w-[180px]">
                            <div className="truncate" title={ln.BK_NM}>{ln.BK_NM}</div>
                          </td>
                          <td className="px-1.5 py-0.5 border-r border-[#dde4ed]">{ln.BAR_CD}</td>
                          <td className="px-1.5 py-0.5 border-r border-[#dde4ed]">{ln.WRITER}</td>
                          <td className="px-1.5 py-0.5 text-right border-r border-[#dde4ed]">{ln.OUT_DANGA.toLocaleString()}</td>
                          <td className={`px-1.5 py-0.5 text-right border-r border-[#dde4ed] ${stockLow?'text-red-600 font-bold':''}`}>{ln.BK_QTY10}</td>
                          <td className="px-1.5 py-0.5 text-right border-r border-[#dde4ed]">-</td>
                          <td className="px-1.5 py-0.5 text-right border-r border-[#dde4ed]">-</td>
                          <td className={`px-1.5 py-0.5 text-right border-r border-[#dde4ed] ${stockLow?'text-red-600 font-bold':''}`}>{ln.JG_QTY}</td>
                          <td className="px-1.5 py-0.5 text-center font-bold border-r border-[#dde4ed]">{ln.OD_QTY}</td>
                          <td className="px-1.5 py-0.5 text-center border-r border-[#dde4ed]">{ln.OUT_RATE}</td>
                          <td className="px-1.5 py-0.5 text-right border-r border-[#dde4ed]">{ln.OD_AMT.toLocaleString()}</td>
                          <td className="px-1.5 py-0.5 border-r border-[#dde4ed] text-slate-500 max-w-[120px] truncate">{ln.PUB_REMK}</td>
                ...
                {/* ── 신규 입력 행 (→) ── */}
                {canEdit && (
                  <tr className="bg-[#eff6ff] border-b border-[#c8d8ed]">
                    <td className="px-1.5 py-0.5 text-center text-[#2b579a] font-bold border-r border-[#c8d8ed]">→</td>
                    <td className="px-1 py-0.5 border-r border-[#c8d8ed]">{newLine.BK_CD}</td>
                    <td className="px-1 py-0.5 border-r border-[#c8d8ed] relative min-w-[160px]">
                      <input ref={bookNameRef} value={bookDisplay}
                        ...
                        className="h-5 px-1 text-xs border border-[#8aacda] rounded w-full bg-white focus:outline-none focus:border-indigo-400" />
                    ...
                    <td className="px-1 py-0.5 border-r border-[#c8d8ed]">{newLine.BAR_CD}</td>
                    <td className="px-1 py-0.5 border-r border-[#c8d8ed]">{newLine.WRITER}</td>
                    <td className="px-1 py-0.5 border-r border-[#c8d8ed]">
                      <input ref={outDangaRef} type="number" value={newLine.OUT_DANGA}
                        onChange={e=>{const v=+e.target.value;setNewLine(p=>({...p,OUT_DANGA:v,OD_AMT:calcAmt(v,p.OUT_RATE,p.OD_QTY)}))}}
                        onKeyDown={e=>{ if(e.key==='Enter'){ e.preventDefault(); odQtyRef.current?.focus() }}}
                        className="h-5 px-1 text-xs border border-[#8aacda] rounded w-16 text-right bg-white focus:outline-none" />
                    </td>
                    <td className="px-1 py-0.5 text-right border-r border-[#c8d8ed]">{newLine.BK_QTY10||0}</td>
                    <td className="px-1 py-0.5 text-right border-r border-[#c8d8ed]">-</td>
                    <td className="px-1 py-0.5 text-right border-r border-[#c8d8ed]">-</td>
                    <td className="px-1 py-0.5 text-right border-r border-[#c8d8ed]">{newLine.JG_QTY||0}</td>
                    <td className="px-1 py-0.5 border-r border-[#c8d8ed]">
                      <input ref={odQtyRef} type="number" value={newLine.OD_QTY}
                        onChange={e=>{const v=+e.target.value;setNewLine(p=>({...p,OD_QTY:v,OD_AMT:calcAmt(p.OUT_DANGA,p.OUT_RATE,v)}))}}
                        onKeyDown={e=>{ if(e.key==='Enter'){ e.preventDefault(); outRateRef.current?.focus() }}}
                        className="h-5 px-1 text-xs border border-[#8aacda] rounded w-12 text-center bg-white focus:outline-none" />
                    </td>
                    <td className="px-1 py-0.5 border-r border-[#c8d8ed]">
                      <input ref={outRateRef} type="number" value={newLine.OUT_RATE}
                        onChange={e=>{const v=+e.target.value;setNewLine(p=>({...p,OUT_RATE:v,OD_AMT:calcAmt(p.OUT_DANGA,v,p.OD_QTY)}))}}
                        onKeyDown={e=>{ if(e.key==='Enter'){ e.preventDefault(); pubRemkRef.current?.focus() }}}
                        className="h-5 px-1 text-xs border border-[#8aacda] rounded w-12 text-center bg-white focus:outline-none" />
                    </td>
                    <td className="px-1 py-0.5 text-right border-r border-[#c8d8ed]">{newLine.OD_AMT.toLocaleString()||0}</td>
                    <td className="px-1 py-0.5 border-r border-[#c8d8ed]">
                      <input ref={pubRemkRef} value={newLine.PUB_REMK} onChange={e=>setNewLine(p=>({...p,PUB_REMK:e.target.value}))}
                        onKeyDown={e=>{ if(e.key==='Enter'){ e.preventDefault(); saveNewLine() }}}
                        className="h-5 px-1 text-xs border border-[#8aacda] rounded w-full bg-white focus:outline-none" />
                    </td>
                    <td className="px-1 py-0.5 text-center">
...
function L({ children }: { children: React.ReactNode }) {
  return <span className="text-[13px] text-[#4a5a6e] whitespace-nowrap text-right pr-0.5 font-medium">{children}</span>
}
function ToolBtn({ icon, label, onClick, disabled, className='' }: {
  ...
  return (
    <button onClick={onClick} disabled={disabled}
      className={`flex items-center gap-1.5 h-8 px-3 text-[13px] font-medium bg-white border border-[#b8c4d4] rounded-sm hover:bg-[#eef2f7] hover:border-[#8a9ab0] disabled:opacity-40 disabled:cursor-default transition-colors ${className}`}>
